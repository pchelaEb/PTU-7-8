(function (w, d){
  ...
  u = prot + '://share.pluso.ru', uid="1371987591420",
...
,[["",".pluso-overlay-box li a b:active"],"background:-webkit-gradient(linear, 0 0, 0 100%, from(#20a517), to(#95e488));background: -moz-linear-gradient(top, #20a517, #95e488);background:-o-linear-gradient(top, #20a517, #95e488);-ms-filter: progid:DXImageTransform.Microsoft.gradient(startColorStr=#20a517, endColorStr=#95e488);filter: progid:DXImageTransform.Microsoft.gradient(startColorStr=#20a517, endColorStr=#95e488);"]];
  ...
  pluso.html = function(elem, debug)
  {
      ...
      if (typeof pluso.sets[uniq]=='undefined')
      {
        ...
        css.push('.'+uniq+' .pluso-wrap a{ background-image:url('+u+'/i/'+p.join('/')+'/'+o['theme']+'.png) }');

        for(var i=0;i<pluso.buy.length;i++)
        {
          css.push('.'+uniq+' .pluso-wrap a.pluso-market-'+pluso.buy[i]+', .'+uniq+' .pluso-wrap a.pluso-market-'+pluso.buy[i]+'-animate{ background:url('+u+'/i/'+p.join('/')+'/market.png) -'+(i*x)+'px 0}');
      ...
      if (!pluso.services)
      {
        ...
        css.push('.pluso-box a.pluso-market b{ background:url('+u+'/i/round/small/market.png) -21px 0}');
  ...
  pluso.setCounter = function (c, url, buy, counters, advType)
  {
    ...
    var runAdvertisement = function (typeBanner) {
      ...
      function AdFox_getCodeScript() {
        ...
        if (iframeDocument) {
          iframeDocument.write('<scr'+'ipt type="text/javascript">setTimeout(function(){ document.close(); }, 10);<\/script><scr'+'ipt type="text/javascript" src="'+requestSrc+dynamicParameters+'"><\/scr'+'ipt>');
      ...
      AdFox_getCodeScript(typeBanner);
    }
