...
implementation
 ...
 var i,j,por,ch,ch2,loop1,loop2,sum,inv1,inv3,inv4,ValueStr,ValueStol,ValueStr2,ValueStol2:integer;
{$R *.dfm}
...
ShowMessage('Правило заполнения матрицы:для обобщенных сетей справделивы целочисленные значения из интервала -20..20');
...
type matr=array [1..100,1..100] of integer;
  var f:textfile;
   i,j,chedin,chmedin,i1,j1,b,z,m,n,std,nenol,gg,minst,minstr,minstr1,minstr2,stol0,nn,nol,stnenol,k,Cpj,Cqj,h,hh,p,l,t,r,rang1,nnn,rang2,medin,edin,x,y:integer;
    ...
    for hh:=1 to n do
     for y:=1 to std do
      d[hh,y]:=vsp4[hh,y];
        ...
        ch2:=0;loop2:=0;
   for y:=1 to std do begin inc(loop2);
