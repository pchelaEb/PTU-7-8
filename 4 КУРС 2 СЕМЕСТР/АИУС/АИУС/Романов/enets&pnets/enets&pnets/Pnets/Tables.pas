         ...
         for j := 0 to Main.m_net.m_Links.Count - 1 do begin
            ...
            if link.m_A = node then begin
               if link.m_B is TSlot then begin
                  ...
                  with Main.m_Net.m_Links do begin
                     for i1 := 0 to Count - 1 do begin
                        link1 := TLink(Items[i1]);
                        if (link.m_A = link1.m_B) and (link1.m_A = link.m_B) then
                           Form1.GridIncident.Cells[b,s1] := '-' + IntToStr(link.m_Capacity) + '+' + IntToStr(link1.m_Capacity);
                     end;
                  end;
               ...
               if link.m_B is TBarrier then begin
                  ...
                  with Main.m_Net.m_Links do begin
                     for i1 := 0 to Count - 1 do begin
                        link1 := TLink(Items[i1]);
                        if (link.m_A = link1.m_B) and (link1.m_A = link.m_B) then
                           Form1.GridIncident.Cells[b1,s] := '+' + IntToStr(link.m_Capacity) + '-' + IntToStr(link1.m_Capacity);
                     end;
                  end;
