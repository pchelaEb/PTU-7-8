...
type
  ...
  public
    ...
    m_Capacity : Integer;
    // Создание дуги
    constructor Create(net : TNet; A, B : TNode; capacity : Integer; pos : Integer);
    // Прорисовка
    procedure Paint; override;
    // Необходимость перерисовки
    procedure Invalidate; override;
...
constructor TLink.Create(net: TNet; A, B : TNode; capacity : Integer; pos : Integer);
begin
  m_Net := net;
  m_A := A;
  m_B := B;
...
begin
  ...
  AY := TDevice(m_A).m_Pos.Y + m_Pos;
  BX := TDevice(m_B).m_Pos.X;
  BY := TDevice(m_B).m_Pos.Y + m_Pos;
  MX := (AX + BX) shr 1;
  MY := (AY + BY) shr 1;
...
begin
  ...
  WriteLn(TextWriter, na, ' ', nb, ' ', m_Capacity, ' ', m_Pos);
end;
...
begin
  m_Net := net;
  ReadLn(TextReader, na, nb, m_Capacity, m_Pos);
