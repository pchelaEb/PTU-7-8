...
type
  ...
  public
    // Связь A->B
    m_A, m_B : TNode;
    // Вес дуги (количество дуг)
    m_Capacity : Integer;//для таблиц
    // Создание дуги
    constructor Create(net : TNet; A, B : TNode; capacity : Integer);
    // Прорисовка
    procedure Paint; override;
    // Необходимость перерисовки
    procedure Invalidate; override;
  ...
  public
    // Математическое ожидание
    m_MX : Integer;//для таблиц
    // Дисперсия
    m_DX : Integer;//для таблиц
    // Вместимость позиции
    m_Capacity : Integer;//для таблиц
    // Занятость позиции
    m_Initial : Integer;
    // Создание позиции
    constructor Create(net : TNet; X, Y, MX, DX, capacity, initial : Integer);
    // Прорисовка
    procedure Paint; override;
    // Проверка попадания указателя мыши
    function HitTest(x, y : Integer) : Boolean; override;
    ...
    constructor Load(net : TNet; var TextReader : TextFile);
    // Возможность вывода
    function CanAddOutput : Boolean; override;//для таблиц
    // Возможность ввода
    function CanAddInput : Boolean; override;//для таблиц
  end;
...
constructor TLink.Create(net: TNet; A, B : TNode; capacity : Integer);
begin
  m_Net := net;
  m_A := A;
  m_B := B;
...
begin
  ...
  WriteLn(TextWriter, na, ' ', nb, ' ', m_Capacity);
end;
...
begin
  m_Net := net;
  ReadLn(TextReader, na, nb, m_Capacity);
...
constructor TSlot.Create(net : TNet; X, Y, MX, DX, capacity, initial : Integer);
begin
  m_Net := net;
  m_Pos.X := X;
  m_Pos.Y := Y;
...
begin
  inherited;
  WriteLn(TextWriter, m_MX, ' ', m_DX, ' ', m_Capacity, ' ', m_Initial);
end;
...
begin
  ...
  ReadLn(TextReader, m_Pos.X, m_Pos.Y);
  ReadLn(TextReader, m_Name);
  ReadLn(TextReader, m_MX, m_DX, m_Capacity, m_Initial);
  m_Net.m_Nodes.Add(Self);
end;
