...
uses
  Windows, SysUtils, Classes, Graphics, Forms, Contnrs, Math,Dialogs;
...
type
  ...
  public
    ...
    // Приоритет
    m_Priority : Integer;
    //Порог (Необходимое количество меток для срабатывания)
    m_Porog : Integer;
    //необходимый потенциал для срабатывания
    m_Pot : Single;
    // Создание перехода
    constructor Create(net : TNet; X, Y, MX, DX, priority, porog : Integer; pot : Single);
    // Прорисовка
    procedure Paint; override;
    // Необходимость перерисовки
    procedure Invalidate; override;
  ...
  public
    ...
    m_TimeLive : Integer;
    //массив меток
    m_Metki : Array of Single;
    //массив меток
    m_Remake : boolean;
    //отрицательные метки
    m_Mines : boolean;
    // Создание позиции
    constructor Create(net : TNet; X, Y, MX, DX, capacity, initial, timelive : Integer);
    // Прорисовка
    procedure Paint; override;
    // Проверка попадания указателя мыши
    function HitTest(x, y : Integer) : Boolean; override;
...
function CanAddOutput : Boolean;                //*************************************
begin
  result := true;
end;


function CanAddInput : Boolean;                //***************************************
begin
  result := true;
end;
...
constructor TBarrier.Create(net : TNet; X, Y, MX, DX, priority, porog: Integer; pot : Single);
begin
  m_Net := net;
  m_Pos.X := X;
  m_Pos.Y := Y;
...
begin
  ...
  with m_Net.m_Canvas do begin
    ...
    Rectangle(m_Pos.X - 2, m_Pos.Y - 10, m_Pos.X + 2, m_Pos.Y + 10);
     if m_Porog > 1 then begin  //если это нейронный переход
       Brush.Color := clYellow;
       Rectangle(m_Pos.X - 3, m_Pos.Y - 10, m_Pos.X + 3, m_Pos.Y + 10);
     end;
  end;
end;
...
begin
  inherited;
  WriteLn(TextWriter, m_MX, ' ', m_DX, ' ', m_Priority, ' ', m_Porog, ' ', m_Pot);
end;
...
begin
  ...
  ReadLn(TextReader, m_MX, m_DX, m_Priority{, m_Porog});
  //ShowMessage('d');//IntToStr(m_Porog));
  m_Net.m_Nodes.Add(Self);
end;
...
constructor TSlot.Create(net : TNet; X, Y, MX, DX, capacity, initial, timelive : Integer);
var
i : Integer;
begin
  m_Net := net;
...
begin
  ...
  if m_Initial>0 then
   begin
    for i:=0 to m_Initial-1 do
     s:=s+FloatToStr(m_Metki[i])+';';
    //delete(s,1,1);
   end;
  if m_Remake then r:='1' else r:='0';
  if m_Mines then m := '1' else m := '0';
  WriteLn(TextWriter, m_MX, ' ', m_DX, ' ', m_Capacity, ' ', m_Initial, ' ', m_TimeLive, ' ', r, ' ', m, ' ', s);
end;
...
begin
  ...
  ReadLn(TextReader, m_Pos.X, m_Pos.Y);
  ReadLn(TextReader, m_Name);
  ReadLn(TextReader, m_MX, m_DX, m_Capacity, m_Initial, m_TimeLive, r, m, s);
  m_Remake:=r=1;
  m_Mines:=m=1;
  setlength(m_Metki,0);
  if s<>' ' then
   while length(s)>0 do
    begin
     ...
     m_Metki[length(m_Metki)-1]:=StrToFloat(copy(s,1,i-1));
     delete(s,1,i);
    end;
  m_Net.m_Nodes.Add(Self);
end;

function TSlot.CanAddOutput: Boolean;          //****************************
begin
  Result := true;
end;


function TSlot.CanAddInput: Boolean;          //******************************
begin
  Result := true;
end;
